<template>
    <div class="allTemplate purchaseContractImported">
                                ...
                                <el-row :gutter="20">
                                    ...
                                    <el-col :span="6">
                                        <el-form-item label="对人民币汇率" prop="rateCurrency" class="contract-auitTitle">
                                            <el-input v-number-only:10.abs="8" v-model="tempAll.rateCurrency" size="mini" rmbBlur @input="rmbBlurTex"></el-input>
                                        </el-form-item>
                                    </el-col>
                                </el-row>
                                <el-row :gutter="20">
                        ...
                        <div class="table">
                           <common-table
                            ...
                            >
                                ...
                                <template slot-scope="scope" slot="default">
                                    <div v-if="scope.text == 'deliveryDate'">
                                        <el-date-picker size="mini" v-model="arrivalPlanList[scope.index]['deliveryDate']" type="date"  :editable="false" placeholder="选择日期" :picker-options="pickerOptions1" @change="handleChange" ref="datePicker">
                                        </el-date-picker>
                                    </div>
...
import {
        doCreateCodPo,
    } from '@/api/purchase/purchase-contract-imported/purchase-contract-imported-add'
...
import CountrySelect from '@/components/common-select/country-select.vue' //国家
    ...
    const arrivalPlanHeaders = [
        ...
        { prop: 'deliveryDate', label: '交货日期', width: '483px',align:'center', template: 'deliveryDate',required: true ,filter:'deliveryDate'},
        { prop: 'deliveryQty', label: '订单数量',align:'center' ,editable: true,required: true},
    ...
    export default {
        ...
        methods:{
            ...
            toCancel(){//取消
                ...
                this.refresh();
                // this.$router.push({path: '/purchase/PurchasContractImported/:type'});
                this.$store.dispatch('toggleCurrentView',{
                    PurchaseContractImported: {
                        view: 'index'
                        // params: 跳转过去的子组件的属性对象
            ...
            childEventHandlerCurrency(val){//币别
                if(val){
                    ...
                    this.tempAll.rateCurrency = Number(val.exchangeRate).toFixed(8);//对人民币税率
            ...
            childEventHandleCountry(val){//贸易国别
                console.log(val,"val");
                if (val) {
            ...
            payTypeChange(){
                ...
                for(var i = 0; i < this.paymentMethodList.length - 1; i++){
                    if(this.payIndex != i){
            ...
            priceTexBlur(row){
                setTimeout(() => {
                    if(row.itemQty && row.orginPriceTex){
                        ...
                        row.baseAmt = row.baseAmtTex;//本币金额
            ...
            //公共计算
            showTax(row){
                // row.addTexAmt = toDecimal2(row.addTexAmt)*row.priceTex;
                if(this.commodityDetailedList.length > 0){
                    let tariffAmt = 0;
                    let addTexAmt = 0;
                    ...
                    this.tempAll.baseAmtTex = baseAmtTex.toFixed(2); //本币合计金额(含税)
                    this.tempAll.baseAmt = baseAmtTex.toFixed(2); //本币合计金额(不含税)
...
<style>
    .purchaseContractImported .top-from{
        /* border-bottom:1px solid #ccc; */
        margin-bottom:10px;
        height:205px;
    }
    .purchaseContractImported .top-from .header-from{
        margin-bottom:2px;
    }
    .purchaseContractImported .planbtn{
        margin-bottom:6px;
    }
    .purchaseContractImported .contract-auitTitle .el-form-item__content{
        margin-left:80px;
    }
    .purchaseContractImported .contract-remark .el-input--mini .el-input__inner{
        width:463px;
    }
    .purchaseContractImported .tab-radio .el-radio__label{
        /* padding-left: 32px; */
        display:none;
    }
    .purchaseContractImported .table-from .el-input__inner{
        height:24px;
        border-radius:0;
    }
</style>
