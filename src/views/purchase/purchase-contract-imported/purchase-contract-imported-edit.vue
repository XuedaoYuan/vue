                ...
                <div v-show="searchBarFlg" class="top-from">
                    <el-tabs v-model="activeName"  @tab-click="handleClick">
                        <el-tab-pane label="抬头数据" name="first">
                             <el-form ref="tempAll" label-width="100px" :model="tempAll" class="header-from"  :rules="rules">
                                <el-row :gutter="20">
                                    <el-col :span="6">
                                ...
                                <el-row :gutter="20">
                                    ...
                                    <el-col :span="6">
                                        <el-form-item label="贸易国别" prop="tradeZoneName">
                                            ...
                                            <country-select @childevent="childEventHandleCountry" :querySelect="tempAll.tradeZoneName"></country-select>
                                        </el-form-item>
                                    </el-col>
                                    <el-col :span="6">
                                        <el-form-item label="溢短装数量" prop="lessQty">
                                            <el-input v-number-only:10.abs="0" v-model="tempAll.lessQty" size="mini" @click="lessBlur"></el-input>
                                        </el-form-item>
                                    </el-col>
                                    <el-col :span="6">
                                        <el-form-item label="对人民币汇率" prop="rateCurrency" class="contract-auitTitle">
                                            <el-input v-number-only:10.abs="8" v-model="tempAll.rateCurrency" size="mini" @blur="rmbBlur" @input="rmbBlurTex"></el-input>
                                        </el-form-item>
                                    </el-col>
                                </el-row>
                                <el-row :gutter="20">
                                     <el-col :span="6">
                                        <el-form-item label="交货期限" prop="deliveryDateStart">
                                            <el-date-picker v-model="deliveryDate" type="daterange" start-placeholder="开始日期" :picker-options="pickerOptions" :editable="false" end-placeholder="结束日期" size="mini" >
                                            </el-date-picker>
                                        </el-form-item>
                                    </el-col>
                                    <el-col :span="6">
                           ...
                           <common-table
                            ...
                            <template slot-scope="scope" slot="default">
                                ...
                                <div v-if="scope.text == 'itemQty'">
                                    <el-input v-number-only:10.abs="2" v-model.number="commodityDetailedList[scope.index]['itemQty']" size='mini'  @input="itemQtyBlur(commodityDetailedList[scope.index])" @blur="itemQtyToFixed(commodityDetailedList[scope.index])"></el-input>
                                </div>
                                <!-- 原币单价 -->
                                ...
                                <div v-if="scope.text == 'orginAmtTex'">
                                    <el-input v-number-only:10.abs="2" v-model.number="commodityDetailedList[scope.index]['orginAmtTex']" size='mini'  @input="baseAmtTexBlur(commodityDetailedList[scope.index])" @blur="baseAmtTexToFixed(commodityDetailedList[scope.index])"></el-input>
                                </div>

                                <!-- 含税金额 -->
                              <!--   <div v-if="scope.text == 'basePriceTex'">
                                    <el-input v-model.number="commodityDetailedList[scope.index]['basePriceTex']" size='mini'  @input="basePriceBlur(commodityDetailedList[scope.index])" @blur="basePriceToFixed(commodityDetailedList[scope.index])"></el-input>
                                </div> -->
                                <!-- 关税税率 -->
                                <div v-if="scope.text == 'tariffRate'">
                                    <el-input v-number-only:10.abs="2" v-model.number="commodityDetailedList[scope.index]['tariffRate']" size='mini'  @input="tariffRateBlur(commodityDetailedList[scope.index])" @blur="tariffRateToFixed(commodityDetailedList[scope.index])"></el-input>
                                </div>

                                <!-- 关税金额 -->
                                <!-- <div v-if="scope.text == 'tariffAmt'">
                                    <el-input v-model.number="commodityDetailedList[scope.index]['tariffAmt']" size='mini'  @input="tariffAmtBlur(commodityDetailedList[scope.index])" @blur="tariffAmtToFixed(commodityDetailedList[scope.index])"></el-input>
                                </div> -->

                                <!-- 增值税税率 -->
                                <div v-if="scope.text == 'addTexRate'">
                                    <el-input v-number-only:10.abs="2" v-model.number="commodityDetailedList[scope.index]['addTexRate']" size='mini' @input="texRateBlur(commodityDetailedList[scope.index])" @blur="texRateToFixed(commodityDetailedList[scope.index])"></el-input>
                                </div>
                                <!-- 增值税金额 -->
                                <!-- <div v-if="scope.text == 'texAmt'">
                                    <el-input v-model.number="commodityDetailedList[scope.index]['texAmt']" size='mini'  @input="texAmtBlur(commodityDetailedList[scope.index])" @blur="texAmtToFixed(commodityDetailedList[scope.index])"></el-input>
                                </div> -->
                            </template>
                            </common-table>
                        </div>
                        ...
                        <div class="table">
                           <common-table
                            ...
                            >
                                ...
                                <template slot-scope="scope">
                                    <div v-if="scope.text == 'deliveryDate'">
                                        <el-date-picker size="mini" v-model="arrivalPlanList[scope.index]['deliveryDate']" type="date" placeholder="选择日期" @change="handleChange" ref="datePicker" :picker-options="pickerOptions1">
                                        </el-date-picker>
                                    </div>
...
import {
    ...
    } from '@/api/purchase/purchase-contract-imported/purchase-contract-imported-edit';
import {pickerOptions,imtAmtTexBur,imtTaxTexBur} from '@/util';
...
import CountrySelect from '@/components/common-select/country-select.vue' //国家
    ...
    const commodityDetailedHeaders = [
        ...
        { prop: 'orginPriceTex', label: '原币单价', width: '120px',align:'center',template: 'orginPriceTex', required: true,digit:'6'},
        { prop: 'basePriceTex', label: '本币单价', width: '120px',align:'center',digit:'6'},
        { prop: 'orginAmtTex', label: '原币金额', width: '120px',align:'center',template: 'orginAmtTex', digit:'2'},
        { prop: 'baseAmtTex', label: '本币金额', width: '120px',align:'center',digit:'2'},
        { prop: 'tariffRate', label: '关税税率', width: '120px',align:'center',template: 'tariffRate', required: true,digit:'2'},
        { prop: 'tariffAmt', label: '关税金额', width: '120px',align:'center',digit:'2'},
        { prop: 'addTexRate', label: '增值税税率', width: '120px',align:'center',template: 'addTexRate', required: true,digit:'2'},
        { prop: 'addTexAmt', label: '增值税金额', width: '120px',align:'center',digit:'2'},
        // { prop: 'orginPriceTex', label: '含税单价', width: '120px',align:'center',template: 'orginPriceTex', required: true}, //无
        // { prop: 'texRate', label: '税率', width: '120px',align:'center' ,template:'texRate', required: true},
        // { prop: 'orginPrice', label: '无税单价', width: '120px',align:'center'},
        // { prop: 'basePriceTex', label: '含税金额', width: '120px',align:'center',template:'basePriceTex' }, //无
        // { prop: 'orginAmtTex', label: '税前金额', width: '120px',align:'center'}, //无
        // { prop: 'orginTexAmt', label: '税额', width: '120px',align:'center'},
    ...
    const arrivalPlanHeaders = [
        ...
        { prop: 'deliveryDate', label: '交货日期', width: '483px',align:'center', template: 'deliveryDate',filter:'deliveryDate', required: true },
        ...
        data(){
            return {
                ...
                rules:{
                    ...
                    rateCurrency:[{required: true, message: ' ', trigger: 'change'}],//对人民币税率
                },
        ...
        methods:{
                        ...
                        if(response.data.status == 1){
                            ...
                            if(headerForm != null){
                                ...
                                this.tempAll = {
                                    ...
                                    tradeZoneCode:headerForm.tradeZoneCode,//贸易国别
                                    tradeZoneId:headerForm.tradeZoneId,//贸易国别
                                    ...
                                    baseAmtTex:headerForm.baseAmtTex,//本币合计金额(不含税)
                                    baseAmt:headerForm.baseAmt,//本币合计金额(含税)
                                    baseTexAmt:headerForm.baseTexAmt,//本币税额
                                    orginTexAmt:headerForm.orginTexAmt,//原币税额
                                };
                                this.deliveryDate = [];
                                ...
                                if(this.tempAll.transType != null){
                                    this.tempAll.transType = String(this.tempAll.transType);
                                }else{
                                    this.tempAll.transType = '';
                                }

                                this.tempAll.rateCurrency = String(this.tempAll.rateCurrency);
                                                      ...
                                                      //付款方式
                                                        ...
                                                        this.showTax();
                                                         if(orginTex != Number(this.tempAll.orginTex)){
                                                            this.$message({
                                                                message: '付款方式原币金额汇总应等于商品明细原币金额汇总',
                                                                type: 'warning'
                                                            });
                                                            return;
                                                        }
            ...
            getChildevent(val){//供应商
                if(val){
                    ...
                    this.$refs['tempAll'].validateField('supplyName');
                }else {
                    this.tempAll.supplierName = '';
                    this.tempAll.supplierCode = '';
                    this.tempAll.supplierId = '';
            ...
            childEventHandlerCurrency(val){//币别
                if(val){
                    ...
                    this.$refs['tempAll'].validateField('currencyName');
                }else {
            ...
            childEventHandleCountry(val){
                 if (val) {
                    this.tempAll.tradeZoneName = val.countryName;
                    this.tempAll.tradeZoneCode = val.countryCode;
                    this.tempAll.tradeZoneId = val.countryId;
                } else {
                    this.tempAll.tradeZoneName = '';
                    this.tempAll.tradeZoneCode = '';
                    this.tempAll.tradeZoneId = '';
                }
            },
            rmbBlur(){
                if(this.tempAll.rateCurrency != ''){
                    this.tempAll.rateCurrency = Number(this.tempAll.rateCurrency).toFixed(8);
                }
            },
            lessBlur(){
                if(this.tempAll.lessBlur != ''){
                    this.tempAll.lessBlur = Number(this.tempAll.lessBlur).toFixed(2);
                }
            },
            ...
            // childEventHandlerCurrencyTab(val){
            //   if(val){
            ...
            priceTexToFixed(row){//原币单价
                if(row.orginPriceTex != ''){
                    row.orginPriceTex = Number(row.orginPriceTex).toFixed(6);//原币单价
                }
            },
            texRateToFixed(row){//增值税税率
                if(row.addTexRate != ''){
                    row.addTexRate = Number(row.addTexRate).toFixed(2);//增值税税率
                }
                if(row.addTexRate > 1){
                    this.$message({
                        message: '增值税税率大于0小于1',
            ...
            itemQtyToFixed(row){//数量
                if(row.itemQty != ''){
                    row.itemQty = Number(row.itemQty).toFixed(2);//数量
                }
            },
            tariffRateToFixed(row){//关税税率
                if(row.tariffRate != ''){
                    row.tariffRate = Number(row.tariffRate).toFixed(2);//关税税率
                }
                if(row.tariffRate > 1){
                    this.$message({
                        message: '增值税税率大于0小于1',
            ...
            rmbBlurTex(){//对人民币汇率
                let rateCurrency = this.tempAll.rateCurrency;
                let list = this.commodityDetailedList;
                setTimeout(() => {
                    if(rateCurrency){
                        let tariffAmt = 0;
                        let addTexAmt = 0;
                        list.forEach((o,i) => {
                            var obj = imtAmtTexBur({"orginPriceTex":o.orginPriceTex,"num":o.itemQty,"rmbCurrencyRate":rateCurrency,"tariffRate":o.tariffRate,"addTexRate":o.addTexRate});
                            console.log(obj);
                            o.orginAmtTex = obj.orginAmtTex;//原币金额
                            o.orginAmt = o.orginAmtTex;//原币金额(不含税)
                            o.basePriceTex = obj.basePriceTex;//本币单价
                            o.basePrice = obj.basePriceTex;//本币单价(不含税)
                            o.baseAmtTex = obj.baseAmtTex;//本币金额
                            o.baseAmt = o.baseAmtTex;//本币金额(不含税)
                            o.tariffAmt = obj.tariffAmt;//关税金额
                            o.addTexAmt = obj.texAmt;//增值税金额

                            this.tempAll.orginAmt = obj.amt; //原币合计金额(不含税)
                            this.tempAll.orginTex = obj.amtTex; //原币合计金额(含税)
                            this.tempAll.baseAmtTex = obj.amt; //原币合计金额(不含税)
                            this.tempAll.baseAmt = obj.amtTex; //原币合计金额(含税)

                            tariffAmt += Number(o.tariffAmt);
                            addTexAmt += Number(o.addTexAmt);
                        });
                        this.tempAll.tariff = tariffAmt.toFixed(2);
                        this.tempAll.texRate = addTexAmt.toFixed(2);
                    }
                },10)
            ...
            priceTexBlur(row){
                setTimeout(() => {
                    if(row.itemQty && row.orginPriceTex){
                        var rateCurrency = this.tempAll.rateCurrency;
                        var obj = imtAmtTexBur({"orginPriceTex":row.orginPriceTex,"num":row.itemQty,"rmbCurrencyRate":rateCurrency,"tariffRate":row.tariffRate,"addTexRate":row.addTexRate});
                        console.log(obj);
                        row.orginPrice = row.orginPriceTex.toFixed(6);//原币单价(不含税)
                        row.orginAmtTex = obj.orginAmtTex;//原币金额
                        row.orginAmt = row.orginAmtTex;//原币金额(不含税)
                        row.basePriceTex = obj.basePriceTex;//本币单价
                        row.basePrice = obj.basePriceTex;//本币单价(不含税)
                        row.baseAmtTex = obj.baseAmtTex;//本币金额
                        row.baseAmt = row.baseAmtTex;//本币金额(不含税)
                        row.tariffAmt = obj.tariffAmt;//关税金额
                        row.addTexAmt = obj.texAmt;//增值税金额
                    };
                    this.showTax();
                },10)
            ...
            baseAmtTexBlur(row){//原币金额
                setTimeout(() => {
                    if( row.itemQty && row.orginAmtTex){
                        var rateCurrency = this.tempAll.rateCurrency;
                        var obj = imtTaxTexBur({"orginAmtTex":row.orginAmtTex,"num":row.itemQty,"rmbCurrencyRate":rateCurrency,"tariffRate":row.tariffRate,"addTexRate":row.addTexRate});
                        console.log(obj);
                        row.orginAmt = row.orginAmtTex;//原币金额(不含税)
                        row.orginPriceTex = obj.priceTex;//原币单价
                        row.orginPrice = row.orginPriceTex;//原币单价
                        row.basePriceTex = obj.basePriceTex;//本币单价
                        row.basePrice = obj.basePriceTex;//本币单价(不含税)
                        row.baseAmtTex = obj.baseAmtTex;//本币金额
                        row.baseAmt = row.baseAmtTex;//本币金额(不含税)
            ...
            tariffRateBlur(row){//关税税率
                setTimeout(() => {
                    if(this.tempAll.rateCurrency && row.itemQty && row.orginPriceTex && row.tariffRate){
                        var rateCurrency = this.tempAll.rateCurrency;
                        var obj = imtAmtTexBur({"orginPriceTex":row.orginPriceTex,"num":row.itemQty,"rmbCurrencyRate":rateCurrency,"tariffRate":row.tariffRate,"addTexRate":row.addTexRate});
                        // console.log(obj);
                        row.orginAmtTex = obj.orginAmtTex;//原币金额
                    ...
                    this.$confirm('您确认要删除吗?', '提示', {
                      ...
                      type: 'warning'
                        ...
                        for (var i = 0; i < this.arrivalPlanList.length; i++) {
                            for (var j = 0; j < this.planDeleteList.length; j++) {
                                if (this.planDeleteList[j] == this.arrivalPlanList[i]) {
                                  this.arrivalPlanList.splice(i, 1);
                                  i--;
                                }
                            }
                        };
            ...
            showTax(){
                if(this.commodityDetailedList.length > 0){
                    let tariffAmt = 0;
                    let addTexAmt = 0;
                    let orginAmtTex = 0;//原币合计金额
                    let baseAmtTex = 0;//本币合计金额
                    ...
                    this.commodityDetailedList.forEach((o,i) => {
                        ...
                        orginTexAmt += NP.minus(orginAmtTex,orginAmtTex);
                        baseTexAmt += NP.minus(baseAmtTex,baseAmtTex);
                    });
                    this.tempAll.tariff = tariffAmt.toFixed(2);
                    this.tempAll.texRate = addTexAmt.toFixed(2);

                    this.tempAll.orginAmt = orginAmtTex.toFixed(2); //原币合计金额(不含税)
                    this.tempAll.orginTex = orginAmtTex.toFixed(2); //原币合计金额(含税)
                    this.tempAll.baseAmtTex = baseAmtTex.toFixed(2); //本币合计金额(不含税)
                    this.tempAll.baseAmt = baseAmtTex.toFixed(2); //本币合计金额(含税)
                    this.tempAll.baseTexAmt = baseTexAmt.toFixed(2);//本币税额
                    this.tempAll.orginTexAmt = orginTexAmt.toFixed(2);//原币税额
                }
            }
        },
        filters: {
