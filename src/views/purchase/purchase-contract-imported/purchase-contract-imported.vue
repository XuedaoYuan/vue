    ...
    <div class="allTemplate">
                        ...
                        <el-row :gutter="20">
                            ...
                            <el-col :span="6">
                                <el-form-item label="溢短装数量" prop="lessQty">
                                    <el-input v-model="tempAll.lessQty" size="mini"></el-input>
                                </el-form-item>
                            </el-col>
                        </el-row>
                        <el-row :gutter="20">
                            ...
                            <el-col :span="6">
                                <el-form-item label="创建时间" prop="name">
                                    <el-date-picker v-model="createTime" type="daterange" start-placeholder="开始日期" :editable="false" :picker-options="pickerOptions" end-placeholder="结束日期" size="mini" >
                                    </el-date-picker>
                                </el-form-item>
                            </el-col>
                            <el-col :span="6">
                        ...
                        <el-row :gutter="20">
                            <el-col :span="6">
                                <el-form-item label="签署日期" prop="signDate">
                                    <!-- <el-date-picker type="date" placeholder="开始时间" v-model="tempAll.signDate" style="width: 100%;" :editable="false" size="mini"></el-date-picker> -->
                            ...
                            <el-col :span="6">
                                <el-form-item label="贸易国别" prop="tradeZoneName">
                                    ...
                                    <country-select @childevent="childEventHandleCountry" :querySelect="tempAll.tradeZoneName" ref="tradeZone"></country-select>
                                </el-form-item>
                            </el-col>
                            <el-col :span="6">
                        ...
                        <el-row :gutter="20">
                            <el-col :span="6">
                                <el-form-item label="交货期限" prop="deliveryDateStart">
                                    <el-date-picker v-model="deliveryDate" type="daterange" start-placeholder="开始日期" :editable="false" :picker-options="pickerOptions" end-placeholder="结束日期" size="mini" >
                                    </el-date-picker>
                                </el-form-item>
                            </el-col>
                            <el-col :span="6">
                        ...
                        <el-row :gutter="20">
                            <el-col :span="6">
                                <el-form-item label="修改时间" prop="name">
                                    <el-date-picker v-model="updatedTime" :editable="false" type="daterange" start-placeholder="开始日期" :picker-options="pickerOptions" end-placeholder="结束日期" size="mini" >
                                    </el-date-picker>
                                </el-form-item>
                            </el-col>
                            <el-col :span="6">
                                <el-form-item label="审核时间" prop="name">
                                    <el-date-picker v-model="postDate" :editable="false" type="daterange" start-placeholder="开始日期" :picker-options="pickerOptions" end-placeholder="结束日期" size="mini" >
                                    </el-date-picker>
                                </el-form-item>
                            </el-col>
                        </el-row>
        ...
        <div class="table">
           <common-table
            ...
            @row-db-click="handledbClick"
            @selection-change="handleSelectionChange"
            >
                <template slot="filter" slot-scope="scope">
                    <!-- 合同类型 -->
                    <div v-if="scope.text == 'poType'">
                      <span>{{entranceList[scope.index]['poType'] | poTypeFilter}}</span>
                    </div>
                    <!-- 合同状态 -->
                    <div v-if="scope.text == 'poStatus'">
                      <span>{{entranceList[scope.index]['poStatus'] | poStatusFilter}}</span>
                    </div>
                    <!-- 运输方式 -->
                    <div v-if="scope.text == 'transType'">
                      <span>{{entranceList[scope.index]['transType'] | transTypeFilter}}</span>
                    </div>
                    <!-- 计重标准 -->
                    <div v-if="scope.text == 'weightStandType'">
                      <span>{{entranceList[scope.index]['weightStandType'] | weightStandTypeFilter}}</span>
                    </div>
                    <!-- 提货方式 -->
                    <div v-if="scope.text == 'pickUpType'">
                      <span>{{entranceList[scope.index]['pickUpType'] | pickUpTypeFilter}}</span>
                    </div>
                    <!-- 包装方式 -->
                    <div v-if="scope.text == 'packType'">
                      <span>{{entranceList[scope.index]['packType'] | packTypeFilter}}</span>
                    </div>
                    <!-- 签署日期 -->
                    <div v-if="scope.text == 'signDate'">
                      <span>{{entranceList[scope.index]['signDate'] | parseTime('{y}-{m}-{d}')}}</span>
                    </div>
                    <!-- 交货期限 -->
                    <div v-if="scope.text == 'deliveryDateEnd'">
                      <span>{{entranceList[scope.index]['deliveryDateStart'] | parseTime('{y}-{m}-{d}')}}</span> -
                      <span>{{entranceList[scope.index]['deliveryDateEnd'] | parseTime('{y}-{m}-{d}')}}</span>
                    </div>
                </template>
...
} from 'api/purchase/purchase-contract-imported/purchase-contract-imported.js';
const headers = [
        { type: 'selection', width: '55px',fixed:'left'},
        { prop: 'poNo', label: '系统单据号', width: '151px',align:'center' },
        ...
        { prop: 'deliveryDateEnd', label: '交货期限', width: '120px',align:'center' ,dataType:'Date',filter:'deliveryDateEnd'},
       { prop: 'pickUpType', label: '提货方式', width: '120px',align:'center',filter:'pickUpType'},
        { prop: 'currencyName', label: '币别', width: '120px',align:'center' },
        { prop: 'rateCurrency', label: '对人民币汇率', width: '120px',align:'center' },
        { prop: 'portName', label: '目的港', width: '120px',align:'center' },
        { prop: 'tariff', label: '关税', width: '120px',align:'center' },
        // { prop: 'entrustPartyName', label: '委托方', width: '120px',align:'center' },
        // { prop: 'tariff', label: '委托协议号', width: '120px',align:'center' },//无
...
export default {
    ...
    data(){
      return {
        ...
        tempAll:{
            ...
            createdTimeStart:'',//创建时间--始
            createdTimeEnd:'',//创建时间--终
            transType:'',//运输方式
            ...
            updatedBy:'',//修改人
            auitTitle:'',//OA审核标题
            updatedTimeStart:'',//修改时间--始
            updatedTimeEnd:'',//修改时间--终
    ...
    methods:{
        getList(){
            ...
            doQueryCodPoWithPage(this.listQuery,this.tempAll).then( response => {
                if(response.data.status === 1){
                    this.entranceList = response.data.data.datalist;
        ...
        resetBtn(){
            ...
            this.postDate = [];//审核时间
            this.$refs.tradeZone.clearCustomer();//国家
        },
        ...
        handledbClick(){//双击查看
            ...
            // this.$router.push({params: {type: 'detail'},query: {updateId:this.updateId}})
            this.$store.dispatch('toggleCurrentView',{
                PurchaseContractImported: {
                    view: 'look',
                    params:{
        ...
        childEventHandlerStaff(val){//修改人
            console.log(val,"修改人");
            if (val) {
...
<style>
    .el-input__prefix .el-input--mini .el-input__icon{
        line-height: 24px;
    }
</style>
