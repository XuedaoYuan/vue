<template>
  <div class="allTemplate">
      ...
      <div v-show="searchBarFlg">
        <el-tabs v-model="activeTabs"  @tab-click="handleClick">
              ...
              <el-row :gutter="20">
                ...
                <el-col :span="6">
                  <el-form-item label="采购收货单" prop="poGrNo">
                    <!--<return-received :disabled = 'createdBtnStatus1' @childevent="childEventHandlerReceived" :poGrStatus="formData.postStatus" :queryId="formData.manageSubId" :querySelect="formData.poGrNo"></return-received>-->
                ...
                <el-col :span="6">
                  <el-form-item label="退货类型" prop="roType">
                    <el-select v-model="formData.roType" @change="selectRoType(formData)" clearable placeholder="请选择" size="mini">
              ...
              <el-row :gutter="20">
                ...
                <el-col :span="6">
                  <el-form-item label="部门" prop="bizOrgName">
                    <department-select disabled  @childevent = "childEventHandlerDepartment" :querySelect="formData.bizOrgName"  :disabled="isDepartment" :transferSearch="transferSearchDepartment"></department-select>
                  </el-form-item>
                </el-col>
              </el-row>
              <el-row :gutter="20">
                <el-col :span="24">
                  <el-form-item label="冲销原因" prop="writeOffReason">
                    <el-input v-model="formData.writeOffReason" size="mini" ></el-input>
                  </el-form-item>
                </el-col>
              </el-row>
            </el-form>
          ...
          <el-tab-pane label="系统信息" name="third" class="tabpane">
            <el-form ref="form" label-width="100px" :model="formData" style="padding-top:16px">
              <el-row :gutter="20">
                <el-col :span="8">
    ...
    <div style="margin-top:12px;margin-bottom:6px">
      ...
      <!--<el-button type="warning" size="mini" @click="handleAddNewCell">创建</el-button>-->
      <el-button type="warning" size="mini" @click="handleDeleteClick" icon="el-icon-delete" plain>删 除</el-button>
    </div>
    ...
    >
      ...
      <template slot-scope="scope">
        ...
        <!--<div v-if="scope.text == 'inputPoGrDtlRowNo'">-->
          <!--<el-input v-model="tableData[scope.index]['poGrDtlRowNo']" size='mini'></el-input>-->
        <!--</div>-->
        <!-- 采购收货单行号 -->
        ...
        <!--包装方式-->
        <!--<div v-if="scope.text == 'inputPackType'">-->
          <!--<el-input v-model="tableData[scope.index]['packType']" size='mini'></el-input>-->
        <!--</div>-->
        ...
        <div v-if="scope.text == 'inputRetireQty'">
          <el-input :disabled="isRoType" v-number-only:10.abs="2" @blur="retireQtyBlur(tableData[scope.index])" v-model="tableData[scope.index]['retireQty']" size='mini'></el-input>
        </div>
        ...
        <div v-if="scope.text == 'inputCabinetNo'">
          <el-input :disabled="isRoType" v-model="tableData[scope.index]['cabinetNo']" size='mini'></el-input>
        </div>
        <div v-if="scope.text == 'inputSupplierBatchNo'">
          <el-input :disabled="!isRoType" v-model="tableData[scope.index]['supplierBatchNo']" size='mini'></el-input>
        </div>
...
<script type="text/javascript">
  import { doCusSave ,doQueryWithReturn } from '@/api/purchase/purchase-return/purchase-return.js'
  import { getList } from '../../../api/purchase/purchas-receipt/index.js'
  ...
  export default {
    ...
    data(){
      return {
        ...
        formData: {
          poRoNo: "",
          postStatus: '70400010',//过账状态:默认保存
          poGrNo: '',//采购收货单号
          roType: '73050005',//退货类型
          manageSubCode: '',
          manageSubName: '',//经营主体
          manageSubId: '',
          supplierCode: '',
...
//      this.$refs['saveForm'].clearValidate();//移除整个表单验证
    ...
    methods: {
      ...
      selectRoType(val){
        console.log('roType=>',val.roType)
        if(val.roType == '4'){
          this.isRoType=true
        }else{
          this.isRoType=false
        }
        console.log('isRoType',this.isRoType)
      },
      choosePurse(val){
        ...
        if( this.multipleSelection.length<1){
          ...
          this.$confirm('您确认要删除吗?', '提示', {
            ...
            this.multipleSelection.forEach((x,i) => {
              let idx = this.tableData.findIndex((val) => val.goodsCode == x.goodsCode);
      ...
      fillRow(val){
       console.log('fillRow',val)
        for(let obj of val) {
//          let index = this.tableData.findIndex((x) => {console.log('obj.poGrDtlId', obj.poGrDtlId,x.poGrDtlId);console.log('obj.poGrDtlId', obj,x);obj.poGrDtlId == x.poGrDtlId});
...
//              retireQty:obj.actReturnQty,//实退数量
              qty:obj.qty,
//              settleQty:obj.settleQty,//结算数量
              lessQty:obj.lessQty,
...
//          vm.formData.billDate = vm.formData.billDate.parseTime ('YYYY-MM-DD HH:MM:SS')
              ...
              success: (valid)=>{   //成功回掉
                if (valid) {
                  ...
                  if(vm.formData && vm.tableData){
                    ...
                    doCusSave(vm.formData,vm.tableData).then(response => {
