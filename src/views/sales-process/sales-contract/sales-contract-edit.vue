  ...
  <div class="allTemplate">
                  ...
                  <el-row>
                      ...
                      <el-col :span="6">
                          <el-form-item label="运输方式" prop="transType">
                              <el-select v-model="headerData.transType" placeholder="请选择" size="mini">
                  ...
                  <el-row>
                      <el-col :span="6">
                          <el-form-item label="币别" prop="currencyName">
                              <currency @childevent="childEventHandlerCurrency" :querySelect="headerData.currencyName"></currency>
                      ...
                      <el-col :span="6">
                          <el-form-item label="计重标准" prop="weightStandType">
                            <el-select v-model="headerData.weightStandType" placeholder="请选择" size="mini">
                  ...
                  <el-row>
                      ...
                      <el-col :span="6">
                          <el-form-item label="合同状态" prop="soStatus">
                            <el-input :value="soStatusOfEdit | soStatus" size="mini" disabled></el-input>
                          </el-form-item>
                      </el-col>
                      <el-col :span="6">
                  ...
                  >
                    ...
                    <template slot-scope="scope" slot="default">
                      ...
                      <div v-if="scope.text == 'currencyName'">
                        <currency @childevent="childEventHandlerCurrencyPlan" :querySelect="collectionPlanData[scope.index]['currencyName']"></currency>
                      </div>
                    </template>
                  </common-table>
              </div>
    ...
    <div class="table">
      <common-table
        :data="list"
        :maxHeight="300"
        :fit="true" ref="firstTable"
        :headers="headers" :rowSelection="false"
        ...
        >
          <template slot="filter" slot-scope="scope">
            ...
            <div v-if="scope.text == 'deliveryMark'">
              <el-checkbox :checked="list[scope.index]['checked']" disabled></el-checkbox>
          ...
          <template slot-scope="scope" slot="default">
            ...
            <div v-if="scope.text == 'texRate'">
              <el-input v-number-only:10.abs="2" v-model.number="list[scope.index]['texRate']" size='mini' @blur="texRateBlur(list[scope.index])" @input="texRateBlurIpt(list[scope.index])"></el-input>
            </div>
            <!-- 无税单价 -->
...
<script>
  ...
  export default {
    components:{
      ManagementSubject,CustomerSelect,DepartmentSelect,GoodsSelect,currency,StaffSelect
    },
    props:{
      updateId:'',
    },
    ...
    methods:{
        ...
        if(this.$route.query){
          ...
          doQueryCodSo(id).then(response=> {
            if(response.data.status == 1){
              let headerData = response.data.data.codSoVO;
              let list = response.data.data.codSoDtlVOList;
              let expmVOList = response.data.data.pmVOList;//收款计划
              // 表头数据
              this.headerData = {
                auitTitle:headerData.auitTitle,
                bizOrgCode:headerData.bizOrgCode,
                bizOrgId:headerData.bizOrgId,
                bizOrgName:headerData.bizOrgName,
                buyerCode:headerData.buyerCode,
                buyerId:headerData.buyerId,
                buyerName:headerData.buyerName,
                contacts:headerData.contacts,
                contactsAddress:headerData.contactsAddress,
                contactsPhone:headerData.contactsPhone,
                contractQty:headerData.contractQty.toFixed(2),
                contractQuality:headerData.contractQuality,
                createdBy:headerData.createdBy,
                createdTime:headerData.createdTime,
                currencyCode:headerData.currencyCode,
                currencyId:headerData.currencyId,
                currencyName:headerData.currencyName,
                ...
                deliveryDateEnd:headerData.deliveryDateEnd,
                deliveryDateStart:headerData.deliveryDateStart,
                soContractNo:headerData.soContractNo,
                invoiceAccount:headerData.invoiceAccount,
                invoiceAddress:headerData.invoiceAddress,
                invoiceBank:headerData.invoiceBank,
                invoiceCode:headerData.invoiceCode,
                invoiceId:headerData.invoiceId,
                invoiceName:headerData.invoiceName,
                invoicePhone:headerData.invoicePhone,
                isConfirm:headerData.isConfirm,
                isBack:headerData.isBack,
                lessQty:headerData.lessQty.toString(),
                manageSubCode:headerData.manageSubCode,
                manageSubId:headerData.manageSubId,
                manageSubName:headerData.manageSubName,
                rateCurrency:headerData.rateCurrency.toFixed(8),
                remark:headerData.remark,
                signDate:headerData.signDate,
                soId:headerData.soId,
                soNo:headerData.soNo,
                soStatus:headerData.soStatus,
                sourceType:headerData.sourceType,
                postDate:headerData.postDate,
                postPersonName:headerData.postPersonName,
                transType:headerData.transType,
                updatedBy:headerData.updatedBy,
                updatedTime:headerData.updatedTime,
                weightStandType:headerData.weightStandType,
              ...
              list.forEach((o,i)=> {
                this.list.push({
                  ...
                  unitName:o.unitName,
                  version:o.version,
                  checked:o.deliveryMark,
                })
              })
              //收款计划
              expmVOList.forEach((o,i)=> {
                this.collectionPlanData.push({
                  account:o.account,
              ...
              this.soStatusOfEdit = headerData.soStatus;
              ...
              this.transferSearchDepartment = {
                "companyCode":headerData.manageSubCode,
                "companyId":headerData.manageSubId,
                "companyName":headerData.manageSubName,
                "boolToBtn":true,
              },
              // this.collectionPlanData = this.expmVOList;
              // console.log(this.headerData.buyerName);
              this.deliveryDate.push(this.headerData.deliveryDateStart);//交货日期
              this.deliveryDate.push(this.headerData.deliveryDateEnd);
              this.signDate = parseTime(this.headerData.signDate);
            ...
            for (var i = 0; i < this.list.length; i++) {
              ...
              for (var j = 0; j < this.multipleSelection.length; j++) {
                if (this.multipleSelection[j].$index == v.$index) {
                  if(this.multipleSelection[j].soDtlId){
                    this.deleteRowId.push(
                      this.multipleSelection[j].soDtlId
                    )
                  }
      ...
      save(){
        ...
        if(this.headerData.invoiceAddress == null){
          this.headerData.invoiceAddress = '';
        };
        this.headerData.signDate = parseTime(this.signDate,'{y}-{m}-{d}');
        // console.log(this.headerData,'头');
        // console.log(this.list,'表');
        // console.log(this.deleteRowId,'删行id');
        // console.log(this.collectionPlanData,'收款计划')
        // console.log(this.deleteExpmList,'收款计划删除信息')
                            ...
                            if (valid) {
                                this.$confirm('您确认要修改吗?', '提示', {
                                  ...
                                  type: 'warning',
                                    ...
                                    doUpdateCodSo(this.headerData,this.list,this.deleteRowId,this.collectionPlanData,this.deleteExpmList).then(response=> {
                                      if(response.data.status == 1){
                                        this.$notify({
                                          title: '成功',
                                          message: '修改成功！',
            ...
            for (var i = 0; i < this.collectionPlanData.length; i++) {
              ...
              for (var j = 0; j < this.handleCollectionPlan.length; j++) {
                if (this.handleCollectionPlan[j].$index == v.$index) {
                  if(this.handleCollectionPlan[j].pmId){
                    this.deleteExpmList.push({
                      "pmId":this.handleCollectionPlan[j].pmId,
                      "version":this.handleCollectionPlan[j].version,
                    })
                  }
