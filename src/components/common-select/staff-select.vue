<template>
  <div>
    <el-select style="float:left;" v-model="searchForm.name" filterable remote size="mini" class="common-search-select" reserve-keyword clearable @change="chooseItemChange" @clear="clearCustomer" placeholder="请输入关键词" :remote-method="remoteMethod" @focus="chooseItem" :loading="loading" :disabled="disabled" collapse-tags>
      <el-option v-for="item in options" :key="item.userCode" :label="item.userName" :value="item.userCode">
        <el-row :gutter="20" style=" margin: 0;">
          <el-col :span="6" class="cell" style="margin: 0;">{{ item.userName }}</el-col>
        </el-row>
      </el-option>
    </el-select>
    ...
    <staff-dialog :mainInfoDialogFormVisible="mainInfoDialogFormVisible" @closeItem="closeMainInfoDialog" @setItem="setMainInfoDialog"></staff-dialog>
...
<script>
import StaffDialog from '../../components/common-dialog/staff-dialog.vue';
import { doQueryWithPage } from '@/api/common-select/staff-select'
export default {
  props: {
  ...
  methods: {
    ...
    setOptions(list) {
      if (list) {
        this.options = list.filter(item => {
        	if (item.userName) {
        		return item.userName.toLowerCase().indexOf(this.searchTexts.userName.toLowerCase()) !== -1
        	}
        });
      } else {
    ...
    chooseItemChange() {
      if (this.options.length > 0) {
        for (var i = 0; i < this.options.length; i++) {
          if (this.options[i].userCode == this.searchForm.name) {
    ...
    remoteMethod(query) {
      if (query) {
        ...
        setTimeout(() => {
          this.loading = false;
          this.searchTexts.userName = query;
          doQueryWithPage(this.searchTexts, this.listQuery).then(response => {
            if (response.data.status == 1) {
              this.list = response.data.data.datalist;
              this.setOptions(this.list);
              if (this.options && this.options.length == 1 && this.options[0].userName == query) {
                this.$nextTick(() => {
    ...
    handleSelect(item) {
      this.searchForm.name = item.userName;
      // 选中传出的值
