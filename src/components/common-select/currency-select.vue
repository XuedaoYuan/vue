<template>
  <div >
    <el-select style="float:left;" v-model="searchForm.name" filterable remote size="mini" class="common-search-select" reserve-keyword clearable @change="chooseItemChange" @clear="clearCustomer" placeholder="请输入关键词" :remote-method="remoteMethod" @focus="chooseItem" :loading="loading" :disabled="disabled" collapse-tags>
      <el-option v-for="item in options" :key="item.currencyCode" :label="item.currencyShortname" :value="item.currencyCode">
        <el-row :gutter="20" style=" margin: 0;">
          <el-col :span="6" class="cell" style="margin: 0;">{{ item.currencyShortname }}</el-col>
        </el-row>
      </el-option>
    </el-select>
    ...
    <currency-dialog :mainInfoDialogFormVisible="mainInfoDialogFormVisible" @closeItem="closeMainInfoDialog" @setItem="setMainInfoDialog"></currency-dialog>
...
<script>
import CurrencyDialog from '@/components/common-dialog/currency-dialog.vue';
...
export default {
  ...
  methods: {
    ...
    setOptions(list) {
      if (list) {
        this.options = list.filter(item => {
          if (item.currencyShortname) {
            return item.currencyShortname.toLowerCase().indexOf(this.searchTexts.currencyShortname.toLowerCase()) !== -1
          }
        });
      } else {
    ...
    chooseItemChange() {
      ...
      this.searchTexts.currencyShortname = '';
    ...
    remoteMethod(query) {
      if (query) {
        ...
        setTimeout(() => {
          this.loading = false;
          this.searchTexts.currencyName = query;
          doQueryWithPageforCurrency(this.searchTexts, this.listQuery).then(response => {
            if (response.data.status == 1) {
              this.list = response.data.data.datalist;
              this.setOptions(this.list);
              if (this.options && this.options.length == 1 && this.options[0].currencyShortname == query) {
                this.$nextTick(() => {
    ...
    handleSelect(item) {
      this.searchForm.name = item.currencyName;
      // 选中传出的值
